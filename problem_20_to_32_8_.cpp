...
bool IsLastMonthInYears(short Month) {
	return (Month == 12);
}
stDate IncreseDateByOneDay(stDate Date) {
	if (IsLastDayInMonth(Date)) {
		if (IsLastMonthInYears(Date.Month)) {
			Date.Month = 1;
			Date.Day = 1;
			Date.Year++;
		}
...
stDate IncreaseDateByOneWeek(stDate Date) {
	for (int i = 1; i <= 7; i++) {
		Date = IncreseDateByOneDay(Date); // week =7days
	}
	return Date;
}
...
// check days in date : (31/1/2022 become 28/2/22
...
stDate IncreseDateByXYears(short Years, stDate Date) {
...
int main() {
	...
	Date1 = IncreseDateByXYears(10, Date1);
